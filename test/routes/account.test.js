// npm run secure-mode -> roda todos os testes
// npm run secure-mode account.test -> roda um arquivo específico
// roda na porta 3001
...
// BeforeAll executa uma vez antes de todos os testes e beforeEach executa antes de um teste
beforeEach(async () => {
  const res = await app.services.user.save({name: 'User Account', mail: `${Date.now()}@mail.com`, passwd: '123456'})
  user = {...res[0]}
  user.token =  jwt.encode(user, 'Segredo!')

  const res2 = await app.services.user.save({name: 'User Account2', mail: `${Date.now()}@mail.com`, passwd: '123456'})
  user2 = {...res2[0]}
})
...
test('Não deve retornar uma conta de outro usuário', () => {
  return app.db('accounts')
    ...
    .then((res) => {
      ...
      expect(res.body.error).toBe('Este recurso não pertence ao usuário informado')
    })
})
...
test('Deve remover uma conta', () => {
  return app.db('accounts')
    .insert({name: 'Acc By Delete', user_id: user.id}, ['id'])
