...
describe('Ao tentar salvar uma transferência inválida', () => {
  const validTranfer = { description: 'Regular Transfer', user_id: 10000, acc_ori_id: 10000, acc_dest_id: 10001, amount: 100, date: new Date() }
...
describe('Ao remover uma transferência', () => {
  ...
  test('As transações associadas devem ter sido removidas', () => {
    return app.db('transactions').where({ transfer_id: 1000 })
      .then(result => {
        expect(result).toHaveLength(0)
      })
  })
